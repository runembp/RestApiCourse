...
public class MovieRepository : IMovieRepository
{
    ...
    public async Task<Movie?> GetByIdAsync(Guid id, Guid? userId = null, CancellationToken token = default)
    {
        ...
        var movie = await connection.QueryFirstOrDefaultAsync<Movie>(new CommandDefinition("""
                                                                                                       select m.*,
                                                                                                              ...
                                                                                                              myRating as userRating
                                                                                                       from movies m
                                                                                                       left join ratings r on m.id = r.movieid
                                                                                                       left join ratings myRating on m.id = myRating.movieId and myRating.userid = @UserId
                                                                                                       where id = @Id
    ...
    {
        using var connection = _dbConnectionFactory.CreateConnectionAsync(token).GetAwaiter().GetResult();
    ...
    public async Task<IEnumerable<Movie>> GetAllAsync(Guid? userId = null, CancellationToken token = default)
    {
        ...
        var result = await connection.QueryAsync(new CommandDefinition("""
                                                                       ...
                                                                       left join ratings r on m.id = r.movieid
                                                                       left join ratings myr on m.id = myr.movieId and myr.userid = @userId
                                                                       group by id, userRating, slug, title, yearofrelease
    ...
    public async Task<bool> UpdateAsync(Movie movie, CancellationToken token = default)
    {
        ...
        await connection.ExecuteAsync(new CommandDefinition("""
                                                            ...
                                                            """, new { MovieId = movie.Id }, transaction, cancellationToken: token));
